...
class GRAND(nn.Module):
    def __init__(self, G, hid_dim, n_class, S, K, batchnorm, num_diseases, num_mirnas,
        ...
        self.m_fc1 = nn.Linear(out_dim  + n_class* S, out_dim)
        self.d_fc1 = nn.Linear(out_dim  + n_class* S, out_dim)
        self.m_fc2 = nn.Linear(out_dim  + n_class, out_dim)
        self.d_fc2 = nn.Linear(out_dim  + n_class, out_dim)
    ...
    def forward(self, graph,  diseases, mirnas,  training=True):
        ...
        if training:  # Training Mode
            ...
            for s in range(S-1):
                ...
                feat = th.log_softmax(self.mlp(feat), dim=-1)
                # output_list.append(th.log_softmax(self.mlp(feat), dim=-1))  # Prediction
                feat1 = th.cat((feat0, feat), dim=1)
                feat0 = feat1
            ...
            h_m = self.dropout1(F.elu(self.m_fc2(h_m)))  # （383,64）
            h_d = self.dropout1(F.elu(self.d_fc2(h_d)))
            h = th.cat((h_d, h_m), dim=0)
